...
function openAdd(type: MetaType) {
  ...
  nameForm.value        = ''
  nameDialogTitle.value = type === 'target' ? 'Add Sale Target' : 'Add Invoice Status'
...
function openEdit(type: MetaType, row: { id: string; name: string }) {
  ...
  nameDialogTitle.value = type === 'target' ? 'Edit Sale Target' : 'Edit Invoice Status'
  nameDialog.value      = true
}
